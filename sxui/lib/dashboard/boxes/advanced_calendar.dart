...
class AdvancedCalendar extends StatefulWidget {
  final ValueNotifier<List<Map<String, dynamic>>> logMessages; // Update to a structured format

  const AdvancedCalendar({Key? key, required this.logMessages}) : super(key: key);
...
class _AdvancedCalendarState extends State<AdvancedCalendar> {
  ...
  Map<String, List<Map<String, dynamic>>> _events = {}; // Use String as key and a list of maps for event data
              ...
              if (_eventController.text.isNotEmpty) {
                setState(() {
                  if (_events[dateKey] == null) {
                    _events[dateKey] = []; // Initialize the event list
                  }
                  _events[dateKey]!.add({
                  ...
                  widget.logMessages.value = [
                    ...widget.logMessages.value,
                    {
                      'description': '🌟 USER 1 added an event: "${_eventController.text}" on ${_formatDate(_selectedDate)} at ${TimeOfDay.now().format(context)} ⏰ (Uncompleted)',
                      'completed': false,
                    },
                  ];
      ...
      return AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              setState(() {
                String eventDescription = _events[dateKey]![index]['description'];
                DateTime eventDate = _events[dateKey]![index]['date'];
                TimeOfDay eventTime = _events[dateKey]![index]['time'];

                _events[dateKey]?.removeAt(index); // Remove the event
                if (_events[dateKey]?.isEmpty ?? false) {
                  _events.remove(dateKey); // Remove the date key if no events left
                }
                ...
                widget.logMessages.value = [
                  ...widget.logMessages.value,
                  {
                    'description': '🌟 USER 1 removed an event: "$eventDescription" on ${_formatDate(eventDate)} at ${eventTime.format(context)} ⏰',
                    'completed': false, // Keep it uncompleted for the log
                  },
                ];
                widget.logMessages.notifyListeners(); // Notify listeners immediately
              });
              updateDialogState(() {}); // Update the dialog's state immediately
              Navigator.of(context).pop(); // Close the confirmation dialog
            },
            child: Text('Remove'),
          ),
...
void _toggleEventCompletion(String dateKey, int index) {
  setState(() {
    ...
    if (isCompleted) {
      widget.logMessages.value = [
        ...widget.logMessages.value,
        {
          'description': '🌟 USER 1 marked event as incomplete: "$description" on ${_formatDate(eventDate)} at ${eventTime.format(context)} ⏰',
          'completed': false,
        },
      ];
    } else {
      widget.logMessages.value = [
        ...widget.logMessages.value,
        {
          'description': '🌟 USER 1 marked event as complete: "$description" on ${_formatDate(eventDate)} at ${eventTime.format(context)} ⏰',
          'completed': true,
        },
      ];
    }
  ...
  void _scrollToCurrentDate() {
    ...
    final indexToScroll = startWeekday - 1 + _selectedDate.day; // Index of the current date
    final scrollPosition = (indexToScroll / 7).ceil() * 60.0; // Estimate the scroll position
  ...
  List<Widget> _buildCalendarGrid() {
    ...
    for (int day = 1; day <= daysInMonth; day++) {
      ...
      final isSelected = currentDate.day == _selectedDate.day && currentDate.month == _selectedDate.month && currentDate.year == _selectedDate.year;
      final isToday = currentDate.day == DateTime.now().day && currentDate.month == DateTime.now().month && currentDate.year == DateTime.now().year;
      ...
      dayWidgets.add(
        GestureDetector(
          onTap: () {
            ...
            _showEventsTab(); // Open tab to show events and add a new event
          },
          child: Container(
            ...
            child: Stack(
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                    color: isToday
                        ? Color(0xFFFFD700) // Light gold color for today's date
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            event['description'],
                            style: TextStyle(
                              color: event['completed'] ? Colors.green : Colors.white,
                        ...
                        // Toggle event completion button
                        GestureDetector(
                          onTap: () {
                            setState(() {
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              color: event['completed'] ? Colors.purple : Colors.grey,
                            ),
                            child: AnimatedAlign(
                              duration: Duration(milliseconds: 200),
                  ...
                  ElevatedButton(
                    onPressed: () {
                      // Pass the local setState to _addEvent
                      _addEvent(setState);
                    },
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        // Month and Year Display with Arrows
        Padding(
            ...
            return Expanded(
              child: Center(
                child: Text(
                  day,
                  style: TextStyle(
                    color: (day == 'Sat' || day == 'Sun') ? Color(0xFFFFD700) : Colors.white70, // Light gold for Saturday and Sunday
                    fontSize: 16,
        ...
        SizedBox(height: 4), // Small spacing between days of the week and calendar grid
        // Calendar Grid
        Expanded(
          flex: 3,
